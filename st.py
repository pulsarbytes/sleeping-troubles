...
class States(object):
    ...
    def __init__(self):
        ...
        # load sound icons
        self.icon_music_on, self.rect_icon_music_on = sthelper.load_image('music-on.png', True)
        self.icon_music_off, self.rect_icon_music_off = sthelper.load_image('music-off.png', True)
        self.icon_sound_on, self.rect_icon_sound_on = sthelper.load_image('sound-on.png', True)
        self.icon_sound_off, self.rect_icon_sound_off = sthelper.load_image('sound-off.png', True)
    ...
    def draw(self, screen, background):
        ...
        musicToggleButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 - 1.5*music_icon.get_width(), TOPBAR_HEIGHT/2 - music_icon.get_height()/2, music_icon.get_width(), music_icon.get_height()), bid="music_toggle", normal=music_icon)
        soundToggleButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 + 0.5*sound_icon.get_width(), TOPBAR_HEIGHT/2 - sound_icon.get_height()/2, sound_icon.get_width(), sound_icon.get_height()), bid="sound_toggle", normal=sound_icon)
...
class Menu(States):
    ...
    def draw(self, screen):
        ...
        background.blit(logo, (int(screen.get_width()/2) - int(logo.get_width()/2), int(logo.get_height()/2)))
        
        # add Nick image
        nick_image, rect_nick_image = sthelper.load_image('nick.png', True)
        background.blit(nick_image, (int(screen.get_width()/2) - (int(MENUBUTTON_WIDTH/2) + 110), 2*logo.get_height() + 2*MENUBUTTON_HEIGHT))
        
        # add Eugene image
        eugene_image, rect_eugene_image = sthelper.load_image('eugene.png', True)
        background.blit(eugene_image, (int(screen.get_width()/2) + (int(MENUBUTTON_WIDTH/2) + 110) - eugene_image.get_width(), 2*logo.get_height() + 2*MENUBUTTON_HEIGHT))
        ...
        for i,(button_id,button) in enumerate(menu_buttons.items()):
            ...
            menuButton = pygbutton.PygButton(((int(background.get_width()/2)) - int(MENUBUTTON_WIDTH/2), y_offset, MENUBUTTON_WIDTH, MENUBUTTON_HEIGHT), button[0], button_id, bgcolor=button[1], fgcolor=button[2])
        ...
        if pg.font:    
            ...
            footer = font.render('Sleeping Troubles v.'+ST_VERSION+' - '+ST_YEAR+'', 1, ST_BLACK40)
            footerpos = footer.get_rect(centerx=int(background.get_width()/2), y = screen.get_height() - 30)
...
class Board(object):
    def __init__(self):
        ...
        # icons
        self.icon_home, self.rect_icon_home = sthelper.load_image('home.png', True)
        self.icon_hourglass, self.rect_icon_hourglass = sthelper.load_image('hourglass.png', True)
        self.icon_telephone, self.rect_icon_telephone = sthelper.load_image('telephone.png', True)
        self.icon_hourglass_white, self.rect_icon_hourglass_white = sthelper.load_image('hourglass-white.png', True)
        self.icon_dreamcrystal, self.rect_icon_dreamcrystal = sthelper.load_image('dream-crystal.png', True)
        self.icon_exclamation, self.rect_icon_exclamation = sthelper.load_image('exclamation.png', True)
        self.icon_end, self.rect_icon_end = sthelper.load_image('sleeping.png', True)
...
class Game(States):
    def __init__(self):
        ...
        self.sound_roll_dice_length = sthelper.get_sound_duration('roll-dice.wav')
        self.sound_roll_dice_timer = None
        self.sound_roll_dice.set_volume(0.5)
        
        # inventory
        self.image_tpoint, self.rect_image_tpoint = sthelper.load_image('hourglass.png', True)
        self.image_dcrystal, self.rect_image_dcrystal = sthelper.load_image('dream-crystal.png', True)
        ...
        # images
        self.image_goodnight, self.rect_image_goodnight = sthelper.load_image('goodnight.png', True)
        self.image_hannibal, self.rect_image_goodnight = sthelper.load_image('hannibal.png', True)
        self.image_eugene, self.rect_image_eugene = sthelper.load_image('eugene.png', True)
    ...
    def get_event(self, event, screen):
        ...
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_ESCAPE:
                ...
                screen_rect = pg.Rect(0, 0, screen.get_width(), screen.get_height())
    ...
    def startup(self):
        ...
        if States.game_on == False:
            ...
            self.current_position_coords = self.board.get_square_coords(self.current_position)
    ...
    def update_player_position(self, dice_roll):
        self.next_position = self.player.set_player_position(self.current_position + dice_roll)
        if self.next_position >= len(self.squares):
            self.end = True
            self.next_position = len(self.squares)
        self.current_position_coords = self.board.get_square_coords(self.current_position)
        self.next_position_coords = self.board.get_square_coords(self.next_position)
    ...
    def get_position_feedback(self):
        self.position_type = self.squares[self.current_position]['type'].rsplit(':', 1)
    ...
    def update_message(self):
        if self.position_type[0] == 'normal':
            ...
            if int(self.position_type[1]) > 0:
                self.message = 'Yes! You gain ' + self.position_type[1] + ' Time Points. Roll the dice again.'
            else:
                self.message = 'Hannibal starts barking and you lose ' + self.position_type[1][1] + ' Time Points. Roll the dice again.'
            ...
            self.message = 'You have to move back ' + str(abs(int(self.position_type[1]))) + ' squares unless you have a Dream Crystal to use.'
    ...
    def draw(self, screen, dt):
        ...
        if pg.font:
            ...
            background.blit(message_box_title, (20, int(TOPBAR_HEIGHT/2) - int(message_box_title.get_height()/2) + 2))
            
            if self.message:
                font = pg.font.Font(None, 24)
                self.message_text = font.render(self.message, 1, ST_DARKORANGE)
                background.blit(self.message_text, (100, int(TOPBAR_HEIGHT/2) - int(message_box_title.get_height()/2)))
        ...
        for i,(button_id,button_text) in enumerate(menu_buttons.items()):
            ...
            menuButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 - TOOLBAR_MENUBUTTON_WIDTH/2, y_offset, TOOLBAR_MENUBUTTON_WIDTH, TOOLBAR_MENUBUTTON_HEIGHT), button_text, button_id, bgcolor=ST_LIGHTGRAY)
            menuButton.draw(background)
            if len(self.menuButtons) < len(menu_buttons):
                self.menuButtons.append(menuButton)
        ...
        background.blit(dicebox, (screen.get_width()-(TOOLBAR_WIDTH-10), TOPBAR_HEIGHT + 2*25 + TOOLBAR_MENUBUTTON_HEIGHT))      
        ...
        if self.dice_locked == True:
            ...
            if self.kitchen_stop == True or self.move_back == True or self.take_risk == True or self.telephone_call == True:
                ...
                self.current_dice_image = pg.transform.rotate(current_image, 45)
        else:
            # blit current dice roll
            self.current_dice_image = self.dice_images[self.current_dice_roll][0]
            
        # draw dice image
        background.blit(self.current_dice_image, (screen.get_width() - int(TOOLBAR_WIDTH/2) - int(self.current_dice_image.get_width()/2), 230 - int(self.current_dice_image.get_height()/2)))
        
        # draw dice roll button
        if self.end == False:
            if self.dice_locked == False:
                rollDiceButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 - ROLLBUTTON_WIDTH/2, 280, ROLLBUTTON_WIDTH, ROLLBUTTON_HEIGHT), "ROLL DICE", 'roll-dice', bgcolor=ST_ORANGE, font=pg.font.Font(None, 24))
                rollDiceButton.draw(background)
                if len(self.rollDiceButtons) < 1:
                    self.rollDiceButtons.append(rollDiceButton)
            else:
                rollDiceButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 - ROLLBUTTON_WIDTH/2, 280, ROLLBUTTON_WIDTH, ROLLBUTTON_HEIGHT), "ROLLING...", 'rolling-dice', bgcolor=ST_LIGHTGRAY, font=pg.font.Font(None, 24))
                rollDiceButton.draw(background)
                self.rollDiceButtons = []
        else:
            rollDiceButton = pygbutton.PygButton((screen.get_width() - TOOLBAR_WIDTH/2 - ROLLBUTTON_WIDTH/2, 280, ROLLBUTTON_WIDTH, ROLLBUTTON_HEIGHT), "GAME OVER", 'game-over', bgcolor=ST_LIGHTGRAY, font=pg.font.Font(None, 24))
            rollDiceButton.draw(background)
            self.rollDiceButtons = []
        ...
        if pg.font:
            ...
            inventorytitlepos = inventorytitle.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) - int(inventorytitle.get_width()/2), y = 410)
            background.blit(inventorytitle, (inventorytitlepos.x,inventorytitlepos.y))
            
            # Time points
            tpointsbox = pg.Surface((int(TOOLBAR_WIDTH/2) - 15, 170))
            tpointsbox = tpointsbox.convert()
            tpointsbox.fill(ST_GRAY217)
            background.blit(tpointsbox, (screen.get_width() - (TOOLBAR_WIDTH - 10), 440))      
        
            font = pg.font.Font(None, 18)
            tpointstitle = font.render('Time Points', 1, ST_BLACK100)
            tpointstitlepos = tpointstitle.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) - int(TOOLBAR_WIDTH/4) - int(tpointstitle.get_width()/2), y = 455)
            background.blit(tpointstitle, (tpointstitlepos.x,tpointstitlepos.y))
            background.blit(self.image_tpoint, (background.get_width() - int(TOOLBAR_WIDTH/2) - int(TOOLBAR_WIDTH/4) - int(self.image_tpoint.get_width()/2), 490))
            
            font = pg.font.Font(None, 54)
            tpointscount = font.render(str(self.tpoints), 1, ST_BLACK60)
            tpointscountpos = tpointscount.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) - int(TOOLBAR_WIDTH/4) - int(tpointscount.get_width()/2), y = 560)
            background.blit(tpointscount, (tpointscountpos.x,tpointscountpos.y))
            
            # Dream crystals
            dcrystalsbox = pg.Surface((int(TOOLBAR_WIDTH/2) - 15, 170))
            dcrystalsbox = dcrystalsbox.convert()
            dcrystalsbox.fill(ST_GRAY217)
            background.blit(dcrystalsbox, (screen.get_width() - (int(TOOLBAR_WIDTH/2) - 5), 440))    
            
            font = pg.font.Font(None, 18)
            dcrystalstitle = font.render('Dream Crystals', 1, ST_BLACK100)
            dcrystalstitlepos = dcrystalstitle.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) + int(TOOLBAR_WIDTH/4) - int(dcrystalstitle.get_width()/2), y = 455)
            background.blit(dcrystalstitle, (dcrystalstitlepos.x,dcrystalstitlepos.y))
            background.blit(self.image_dcrystal, (background.get_width() - int(TOOLBAR_WIDTH/2) + int(TOOLBAR_WIDTH/4) - int(self.image_dcrystal.get_width()/2), 490))
            
            font = pg.font.Font(None, 54)
            dcrystalscount = font.render(str(self.dcrystals), 1, ST_BLACK60)
            dcrystalscountpos = dcrystalscount.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) + int(TOOLBAR_WIDTH/4) - int(dcrystalscount.get_width()/2), y = 560)
            background.blit(dcrystalscount, (dcrystalscountpos.x,dcrystalscountpos.y))
        
        # add footer
        if pg.font:
            font = pg.font.Font(None, 16)
            footer = font.render('Sleeping Troubles v.'+ST_VERSION+' - '+ST_YEAR+'', 1, ST_BLACK40)
            footerpos = footer.get_rect(x=background.get_width() - int(TOOLBAR_WIDTH/2) - int(footer.get_width()/2), y = screen.get_height() - 30)
            background.blit(footer, (footerpos.x,footerpos.y))
        
        # draw board
        for key in self.squares:
            if self.squares[key]['coords'] and self.squares[key]['coords'] != []:
                square = pg.Surface((int(self.square_width), int(self.square_height)))
                square = square.convert()
                square.fill(self.squares[key]['bcolor'])
                # draw border
                pg.draw.rect(square, ST_GRAY120, (0,0,int(self.square_width), int(self.square_height)), 1)  
                
                if pg.font and self.squares[key]['text'] != '' and not self.squares[key]['icon']:
                    font = pg.font.Font(None, 22)
                    description = font.render(self.squares[key]['text'], 1, self.squares[key]['fcolor'])
                    square.blit(description, (int(square.get_width()/2) - int(description.get_width()/2), int(square.get_height()/2) - int(description.get_height()/2)))
                
                if self.squares[key]['icon']:
                    if self.squares[key]['text'] == '':
                        y_offset = square.get_height()/2 - self.squares[key]['icon'].get_height()/2
                    else:
                        y_offset = 10
                        if pg.font:
                            font = pg.font.Font(None, 18)
                            text = font.render(self.squares[key]['text'], 1, self.squares[key]['fcolor'])
                            square.blit(text, (int(square.get_width()/2) - int(text.get_width()/2), 55))
                    square.blit(self.squares[key]['icon'], (int(square.get_width()/2) - int(self.squares[key]['icon'].get_width()/2), int(y_offset)))
                
                background.blit(square, (int(self.squares[key]['coords'][0]), int(TOPBAR_HEIGHT) + int(self.squares[key]['coords'][1])))
        
        # draw player
        background.blit(self.player_image, (int(self.next_position_coords[0]) + int(self.square_width/2) - int(self.player_image.get_width()/2), TOPBAR_HEIGHT + int(self.next_position_coords[1]) + int(self.square_height/2) - int(self.player_image.get_height()/2)))
        ...
        if self.telephone_call == True:
            ...
            pg.draw.rect(telephone_popup_bg, ST_BLACK, (0,0,POPUP_WIDTH - 1,POPUP_HEIGHT - 1), 2) 
            background.blit(telephone_popup_bg, (int(screen.get_width()/2) - int(telephone_popup_bg.get_width()/2), int(screen.get_height()/2) - int(telephone_popup_bg.get_height()/2)))
            telephone_popup = pg.Surface((POPUP_WIDTH - 2*POPUP_PADDING, POPUP_HEIGHT - 2*POPUP_PADDING)).convert()
            telephone_popup.fill(ST_WHITE)
            background.blit(telephone_popup, (int(screen.get_width()/2) - int(telephone_popup.get_width()/2), int(screen.get_height()/2) - int(telephone_popup.get_height()/2)))
            font = pg.font.Font(None, 38)
            telephone_title = font.render('YOU HAVE A TELEPHONE CALL', 1, ST_PURPLE)
            background.blit(telephone_title, (screen.get_width()/2 - telephone_title.get_width()/2, screen.get_height()/2 - 100))
            font = pg.font.Font(None, 24)
            telephone_text = font.render('Eugene calls you and has a new task for you.', 1, ST_PURPLE)
            background.blit(telephone_text, (screen.get_width()/2 - telephone_text.get_width()/2, screen.get_height()/2 - 50))           
            ...
            for i,(button_id,button_text) in enumerate(telephone_buttons.items()):
                x_offset = screen.get_width()/2 - telephone_popup.get_width()/2 + 30 + i*POPUP_BUTTON_WIDTH + i*30
                telephoneButton = pygbutton.PygButton((x_offset, screen.get_height()/2, POPUP_BUTTON_WIDTH, POPUP_BUTTON_HEIGHT), button_text, button_id, bgcolor=ST_PURPLE, fgcolor=ST_YELLOW)
        ...
        if self.kitchen_stop == True:
            ...
            pg.draw.rect(kitchen_popup_bg, ST_BLACK, (0,0,POPUP_WIDTH - 1,POPUP_HEIGHT - 1), 2) 
            background.blit(kitchen_popup_bg, (screen.get_width()/2 - kitchen_popup_bg.get_width()/2, screen.get_height()/2 - kitchen_popup_bg.get_height()/2))
            kitchen_popup = pg.Surface((POPUP_WIDTH - 2*POPUP_PADDING, POPUP_HEIGHT - 2*POPUP_PADDING)).convert()
            kitchen_popup.fill(ST_WHITE)
            background.blit(kitchen_popup, (screen.get_width()/2 - kitchen_popup.get_width()/2, screen.get_height()/2 - kitchen_popup.get_height()/2))
            font = pg.font.Font(None, 44)
            kitchen_title = font.render('KITCHEN STOP', 1, ST_DARKORANGE)
            background.blit(kitchen_title, (screen.get_width()/2 - kitchen_title.get_width()/2, screen.get_height()/2 - 100))
            font = pg.font.Font(None, 24)
            kitchen_text = font.render('Select a Bonus', 1, ST_DARKORANGE)
            background.blit(kitchen_text, (screen.get_width()/2 - kitchen_text.get_width()/2, screen.get_height()/2 - 50))           
            ...
            for i,(button_id,button_text) in enumerate(kitchen_buttons.items()):
                x_offset = screen.get_width()/2 - kitchen_popup.get_width()/2 + 30 + i*POPUP_BUTTON_WIDTH + i*30
                kitchenButton = pygbutton.PygButton((x_offset, screen.get_height()/2, POPUP_BUTTON_WIDTH, POPUP_BUTTON_HEIGHT), button_text, button_id, bgcolor=ST_ORANGE)
        ...
        if self.move_back == True:
            ...
            pg.draw.rect(move_back_popup_bg, ST_BLACK, (0,0,POPUP_WIDTH - 1,POPUP_HEIGHT - 1), 2) 
            background.blit(move_back_popup_bg, (screen.get_width()/2 - move_back_popup_bg.get_width()/2, screen.get_height()/2 - move_back_popup_bg.get_height()/2))
            move_back_popup = pg.Surface((POPUP_WIDTH - 2*POPUP_PADDING, POPUP_HEIGHT - 2*POPUP_PADDING)).convert()
            move_back_popup.fill(ST_BLACK)
            background.blit(move_back_popup, (screen.get_width()/2 - move_back_popup.get_width()/2, screen.get_height()/2 - move_back_popup.get_height()/2))
            font = pg.font.Font(None, 30)
            if self.position_type[1] == '-6':
                move_back_title = font.render('WEIRD NOISES COMING FROM THE ATTIC', 1, ST_DARKORANGE)
            elif self.position_type[1] == '-7':
                move_back_title = font.render('YOU FORGOT TO TAKE YOUR VALERIAN PILL', 1, ST_DARKORANGE)
            background.blit(move_back_title, (screen.get_width()/2 - move_back_title.get_width()/2, screen.get_height()/2 - 100))
            font = pg.font.Font(None, 22)
            
            if self.dcrystals > 0:
                move_back_text = font.render('If you have a Dream Crystal you can avoid going back', 1, ST_WHITE)
                ...
                move_back_text = font.render("You don't have a Dream Crystal. You must go back", 1, ST_WHITE)

                move_back_buttons = {
                                    "back:"+self.position_type[1]:"Move " + str(abs(int(self.position_type[1]))) + " squares back"
                                    }
                
            background.blit(move_back_text, (screen.get_width()/2 - move_back_text.get_width()/2, screen.get_height()/2 - 50))           

            for i,(button_id,button_text) in enumerate(move_back_buttons.items()):
                x_offset = screen.get_width()/2 - move_back_popup.get_width()/2 + 30 + i*POPUP_BUTTON_WIDTH + i*30
                moveBackButton = pygbutton.PygButton((x_offset, screen.get_height()/2, POPUP_BUTTON_WIDTH, POPUP_BUTTON_HEIGHT), button_text, button_id, bgcolor=ST_LIGHTGRAY, fgcolor=ST_BLACK)
        ...
        if self.take_risk == True:
            ...
            pg.draw.rect(take_risk_popup_bg, ST_BLACK, (0,0,POPUP_WIDTH - 1,POPUP_HEIGHT - 1), 2) 
            background.blit(take_risk_popup_bg, (screen.get_width()/2 - take_risk_popup_bg.get_width()/2, screen.get_height()/2 - take_risk_popup_bg.get_height()/2))
            take_risk_popup = pg.Surface((POPUP_WIDTH - 2*POPUP_PADDING, POPUP_HEIGHT - 2*POPUP_PADDING)).convert()
            take_risk_popup.fill(ST_RED)
            background.blit(take_risk_popup, (screen.get_width()/2 - take_risk_popup.get_width()/2, screen.get_height()/2 - take_risk_popup.get_height()/2))
            font = pg.font.Font(None, 32)
            take_risk_title = font.render('YOUR TIME POINTS ARE REDUCED TO 1', 1, ST_YELLOW)
            background.blit(take_risk_title, (screen.get_width()/2 - take_risk_title.get_width()/2, screen.get_height()/2 - 100))
            font = pg.font.Font(None, 24)
            take_risk_text = font.render('Will you risk to continue or go back?', 1, ST_YELLOW)
            background.blit(take_risk_text, (screen.get_width()/2 - take_risk_text.get_width()/2, screen.get_height()/2 - 50))           
            ...
            for i,(button_id,button_text) in enumerate(take_risk_buttons.items()):
                x_offset = screen.get_width()/2 - take_risk_popup.get_width()/2 + 30 + i*POPUP_BUTTON_WIDTH + i*30
                takeRiskButton = pygbutton.PygButton((x_offset, screen.get_height()/2, POPUP_BUTTON_WIDTH, POPUP_BUTTON_HEIGHT), button_text, button_id, bgcolor=ST_BLACK, fgcolor=ST_YELLOW)
        ...
        if self.end == True:
            ...
            if self.won_game == True:
                ...
                pg.draw.rect(end_popup_bg, ST_BLACK, (0,0,POPUP_WIDTH - 1,POPUP_HEIGHT - 1), 2) 
                background.blit(end_popup_bg, (screen.get_width()/2 - end_popup_bg.get_width()/2, screen.get_height()/2 - end_popup_bg.get_height()/2))
                end_popup = pg.Surface((POPUP_WIDTH - 2*POPUP_PADDING, POPUP_HEIGHT - 2*POPUP_PADDING)).convert()
                end_popup.fill(ST_WHITE)
                background.blit(end_popup, (screen.get_width()/2 - end_popup.get_width()/2, screen.get_height()/2 - end_popup.get_height()/2))
                background.blit(self.image_goodnight, (screen.get_width()/2 - self.image_goodnight.get_width()/2, screen.get_height()/2 - self.image_goodnight.get_height()/2))           
                ...
                end_popup.fill(ST_WHITE)
                background.blit(end_popup, (screen.get_width()/2 - end_popup.get_width()/2, screen.get_height()/2 - end_popup.get_height()/2))
                background.blit(self.image_hannibal, (screen.get_width()/2 - self.image_hannibal.get_width()/2, screen.get_height()/2 + 30))           
                font = pg.font.Font(None, 44)
                end_title = font.render('GAME OVER', 1, ST_RED)
                background.blit(end_title, (screen.get_width()/2 - end_title.get_width()/2, screen.get_height()/2 - 100))
                font = pg.font.Font(None, 24)
                end_text = font.render('You lost all your Time Points.', 1, ST_RED)
                end_text_2 = font.render('Eugene and Hannibal kept you up all night.', 1, ST_RED)
                background.blit(end_text, (screen.get_width()/2 - end_text.get_width()/2, screen.get_height()/2 - 50))           
                background.blit(end_text_2, (screen.get_width()/2 - end_text_2.get_width()/2, screen.get_height()/2 - 22))           
...
class Pause(States):
    ...
    def draw(self, screen):
        ...
        if pg.font:
            ...
            pausetextpos = pausetext.get_rect(centerx=screen.get_width()/2, y = screen.get_height()/2 - 2 * pausetext.get_height())
            screen.blit(pausetext, (pausetextpos.x,pausetextpos.y))
            
            font = pg.font.Font(None, 26)
            pausesubtext = font.render("Press 'Esc' key to resume game", 1, ST_WHITE)
            pausesubtextpos = pausesubtext.get_rect(centerx=screen.get_width()/2, y = screen.get_height()/2)
            screen.blit(pausesubtext, (pausesubtextpos.x,pausesubtextpos.y))

class Help(States):
    def __init__(self):
        States.__init__(self)
        self.hannibal_image, self.rect_hannibal_image = sthelper.load_image('hannibal.png', True)
    ...
    def draw(self, screen):
        ...
        # draw Hannibal image
        background.blit(self.hannibal_image, (screen.get_width() - TOOLBAR_WIDTH/2 - self.hannibal_image.get_width()/2,screen.get_height() - 180))
        
        if pg.font:
            # add title
            font = pg.font.Font(None, 36)
            title = font.render("Help", 1, ST_BLACK)
            background.blit(title, (50, TOPBAR_HEIGHT/2 - title.get_height()/2))
            ...
            for i,square in enumerate(color_squares):
                colorSquare = pygbutton.PygButton((50,330+i*42,140,32), color_squares[square][0], bgcolor=color_squares[square][1], fgcolor=color_squares[square][3])
            ...
            font = pg.font.Font(None, 16)
            footer = font.render('Sleeping Troubles v.'+ST_VERSION+' - '+ST_YEAR+'', 1, ST_BLACK40)
            footerpos = footer.get_rect(x=background.get_width() - TOOLBAR_WIDTH/2 - footer.get_width()/2, y = screen.get_height() - 30)
            background.blit(footer, (footerpos.x,footerpos.y))
...
class Credits(States):
    def __init__(self):
        States.__init__(self)
        self.rascal_image, self.rect_rascal_image = sthelper.load_image('rascal.png', True)
    ...
    def draw(self, screen):
        ...
        # draw Rascal image
        background.blit(self.rascal_image, (screen.get_width() - TOOLBAR_WIDTH/2 - self.rascal_image.get_width()/2,screen.get_height() - 180))
...
class Control:
    ...
    def main_game_loop(self):
        while not self.done:
            delta_time = self.clock.tick(self.fps)/1000.0 # time between frames
            self.event_loop()
